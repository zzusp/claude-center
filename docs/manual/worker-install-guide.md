...
如果 Console 管理员部署了 SSE relay，有两种配法，二选一即可：

**① 桌面端「设置 → SSE 中转服务」卡片（推荐，免改文件、保存即时生效）**

| 字段 | 说明 |
|---|---|
| 中转服务地址 | relay 基址，如 `https://relay.your-org.com`；**留空 = 禁用、纯轮询** |
| 发布 token | `CLAUDE_CENTER_RELAY_PUBLISH_TOKEN`，向管理员要 |
| 订阅 token | `CLAUDE_CENTER_RELAY_WORKER_TOKEN`，向管理员要 |

填好点「保存」即时重连，无需重启；连通状态见顶栏 SSE 指示。持久化到 `~/.claude-center/worker.json`，**覆盖**同名环境变量。
...
> 桌面端保存过中转配置后即以 `worker.json` 为准（含清空——清空保存表示显式禁用，而非回退到 env）。想改回让 env 生效，删掉 `worker.json` 里的 `relayUrl` / `relayPublishToken` / `relayWorkerToken` 三个键再重启。
