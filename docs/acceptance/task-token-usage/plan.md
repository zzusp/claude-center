...
- `node docs/acceptance/task-token-usage/scripts/verify-token-sort.mjs`：DB 级断言 sort=tokens 升/降序、与 created 排序互相区分、
  total_tokens 回 number、incrementTaskTokens 累加 + claimed_by 守卫 no-op。
- `node docs/acceptance/task-token-usage/scripts/e2e-executor.mjs`：**真·端到端**——临时库 + 临时 git 仓，调产品 `executeTask`
  跑真实 `claude` CLI，证 token 用量被解析累加（实跑 `total_tokens=77901`）、任务 success、改动推到 origin。
- 证据详见 `report.md`。
