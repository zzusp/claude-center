...
任务分类 `task_type`：`work`（默认，改代码/开 PR）/ `qa`（纯对话、不碰 git）。提交模式 `submit_mode`：`pr`（默认，开 PR 到 `target_branch` 后等合并清理）/ `push`（直接 `push` 工作分支到 `target_branch`，落地即 `merged`）。详见 `task-types.md`、`task-branch-submit-mode.md`、`task-cleanup-merge.md`。
...
5. 任务执行（工作类 `work`）：
   - `git fetch origin`、`checkout` + `pull --ff-only` 签出分支 `base_branch`，再 `checkout -B work_branch`
   ...
   - 若产生改动，按 `submit_mode` 分流：`pr` 提交、推送 `work_branch` 并 `gh pr create --base target_branch`，回写 `success` + `pr_url`；`push` 提交并 `git push origin work_branch:target_branch`，落地即回写 `merged`
   - 无改动回写 `success`；异常回写 `failed`
   - 问答类 `qa`：跳过所有 git，只读跑 `claude` 回答 → 答案落评论 + 转 `waiting`（详见 `task-types.md`）
6. 完成后清理（仅 `submit_mode='pr'`）：`gh pr view` 检测到 PR 已合并后，把本地仓库切回签出分支并 `pull`、
