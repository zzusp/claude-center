...
> **已废弃（2026-06-14）**：问答类（qa）已从任务流移除——问答实时性要求高，不应与异步任务流混在一起，
> 后续将独立成单独菜单 + 单独数据通道（指定项目/分支、指定 Worker 直接对话）。`task_type` 字段、qa 分支、
> `completeQaTask`、`qaPrompt` 等已全部删除（迁移 `016_task_drop_type.sql`，方案见
> `docs/acceptance/remove-qa-task-type/plan.md`）。任务流如今只有工作类一种形态。以下为历史快照，仅作记录。
