...
- `parseTestPlan` 纯函数：`docs/acceptance/pr-testplan-gate/scripts/verify-parse-testplan.mts` 用 tsx
  跑 fixture 用例（全勾 / 含未勾 / 含 ❌ / 无 Test Plan / 多标题边界）断言 `allPassed`。
- `npm run typecheck`、`npm run build` 五包全绿。
- 迁移 SQL 按 026/029 既有写法编写；本 worktree 无 `.env`（远程共享库连不上），未对真库跑 `db:ephemeral`，
  在 PR body 注明。
