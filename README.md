...
| **任务有先后依赖，手动协调** | 声明前置任务，前置任务完成后，Worker 自动按顺序认领，不需要再电脑前傻等                 |
...
**非 Git 项目**（详见 `docs/spec/non-git-projects.md`）
- 建项目时可选「非 Git 本地目录」：无需填仓库地址 / 分支，Worker 在本机关联目录里直接运行 Claude、就地修改文件
- 任务与实时对话都支持：无分支 / 提交 / PR，任务完成即成功；其余能力（草稿门禁、定时、自动回复、附件、续接重试）通用
- 注意：非 Git 项目没有 git worktree 隔离，同目录的并发任务会互相影响
...
- 支持随消息添加图片 / 文件附件：粘贴截图、拖拽文件即上传，Worker 落地到只读检出让 Claude 读到（图片走 vision）
- 助手回复以 Claude Code 完整执行会话富展示（工具折叠 / diff 着色 / thinking）
- 定时发送消息：用与任务表单同款的时间控件排定消息到未来某时刻发送，Console 调度器到点入队
- 自动回复（无人值守）：与任务表单同设计，开启后 Claude 自主决策、不停下来反问；新建对话与对话中均可设置
