...
- **套餐用量**（仅套餐账号）：5 小时窗口与 7 天窗口的「已用率 + 重置剩余」，数据来自 `https://api.anthropic.com/api/oauth/usage`（用 access token，经 `CLAUDE_CENTER_USAGE_PROXY` / `HTTPS_PROXY` 代理）。该接口给的是利用率百分比，无绝对额度，故以「已用 X%」表达。
  > **受限网络必看**：这条 usage 请求走的是 `CLAUDE_CENTER_USAGE_PROXY`（缺省回退 worker 进程环境里的 `HTTPS_PROXY`/`HTTP_PROXY`），**与 `CLAUDE_CENTER_CLAUDE_PRE_COMMAND` 的代理是两套出口**——只在 pre-command 里设代理，usage 请求仍直连 `api.anthropic.com`、被挡后返回 `{"error":{"type":"forbidden"}}`，于是套餐账号也采不到用量。国内务必显式配 `CLAUDE_CENTER_USAGE_PROXY`。采集失败时不再静默：worker 日志打印失败原因，Console 详情抽屉的「套餐用量」直接显示「用量采集失败：<原因>」而非空白。
- **并行处理**：当前在途任务列表 + 并行上限。
