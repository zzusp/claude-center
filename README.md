...
- **连通性可见**：Console 顶栏（全站）+ 总览「SSE 连接」卡、Worker 桌面端品牌区都显示 SSE 连接状态（连接中 / 已连通 / 重连中 / 未启用），与「DB 轮询」心跳并列，一眼区分快线是否在用。
...
默认任务在 Claude 中途用哨兵停下后会进入 `waiting` 等人回复。要做到**真正无人托管**（夜里发任务一觉醒来全跑完），需要四项叠加：① 权限姿态（worker 级 `bypassPermissions`，已有）；② 提交入库（`submit_mode=push` 或 `auto_merge_pr=true`，已有）；③ **`tasks.auto_reply=true`（迁移 021 新增，默认 false）**；④ 可选 `tasks.auto_decision_hints`（决策预案文本，喂进 prompt 当用户偏好）。

`auto_reply=true` 时分两层：

- **主防线（提示词）**：把"如需决定请用哨兵停下"翻转为"你在无人值守，遇歧义自己拍板，把决策理由写进 commit message；哨兵被重新定义为'任务被判定 blocked'"。绝大多数偏好/范围/收尾确认型提问被这里拦掉，零额外开销。
- **兜底（仍出哨兵时）**：按 `git status --porcelain` 是否有改动分流——
  - 零改动 → 直接 `markTaskFailed`（任务多半描述太模糊，再问也是同样结果，省 token + 让用户改描述）。
  - 有改动 → 自动塞一条 user 评论 `"Commit what you have and finish in one shot..."`，由现有 resumable 流自然续接；`task_events.event_type='auto_reply'` 计数，**cap=2**（`apps/worker/src/executor.ts` 顶部常量 `AUTO_REPLY_MAX_ROUNDS`），超出 cap 再次出哨兵就以"blocked after N rounds"明确失败，已有改动留在 worktree / 已推送的 PR 供人接管。

Console 任务创建/编辑表单里新增「自动回复」开关，勾选后展开「决策预案」多行文本；详情卡片有对应只读行。详情见 `apps/worker/src/executor.ts` 的 `autoReplyDirective` / `handleClaudeTurn`。
...
- **SSE 连接**：SSE 中转连接状态；admin 可见在线连接、累计事件 / 每秒速率、占用频道。右上角徽章显示与后端的同步心跳（含相对时间）。
